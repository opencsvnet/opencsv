...
//!    → XFERC anchor → consignment to B → `accept()` by B.
...
use opencsv_core::anchor::{AnchorRecord, mint_commit, nullifier_commit};
...
fn full_protocol_flow_with_real_proofs() {
    ...
    let transfer_ref = chain.append(AnchorRecord::XferCompressed {
        nullifier_commit: nullifier_commit(&transfer.statement.nullifiers).to_anchor(),
    });
    chain.advance_blocks(6);
    ...
    // Same consumed coins → same nullifiers → same nullifier commitment key.
    assert_eq!(
        double_spend.statement.nullifiers,
        transfer.statement.nullifiers
    );
    let double_ref = chain.append(AnchorRecord::XferCompressed {
        nullifier_commit: nullifier_commit(&double_spend.statement.nullifiers).to_anchor(),
    });
    chain.advance_blocks(6);
    ...
    assert_eq!(
        ...
        RejectReason::NullifierConflict {
            nullifier: nullifier_commit(&transfer.statement.nullifiers).to_anchor(),
    ...
    let redeem_ref = chain.append(AnchorRecord::Redeem {
        asset_id: asset_id.to_anchor(),
        value: 70,
        nullifier: redeem.statement.nullifiers[0].to_anchor(),
    });
    chain.advance_blocks(6);
